...
class Blog_Entries extends ModelBase
{
	...
	function FindByPagination( $perpage, $pagenum, $public=true )
	{
	    ...
	    // assumes that we need to find entries older than now, not newer than now	
		...
		$query .= ( $public ) ? ' public = 1 AND ' : ' '; 
		$query .= $date.' ORDER BY date DESC '.$max; 
	...
	/*
     * Helper function to fetch photo for an entry
     * @requires an entry object
     * @returns a photo object or false
     */
	function getImage() {
    	$image = null; 
    	$image = array_shift( MyActiveRecord::FindBySql('Photos', "SELECT * FROM photos WHERE entry_id = {$this->id}") );
    	if ( empty($image) ) {
        	return false; 
    	} else {
        	return $image; 
